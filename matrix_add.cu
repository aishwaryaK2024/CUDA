...
__global__ void matAdd2D(float *A, float *B, float *C, int rows, int cols)
{
...
int main(void)
{
    ...
    for (int i = 0; i < N; i++) { h_A[i] = (float)(i+1); h_B[i] = 1.0f; }
    ...
    matAdd2D<<<gridSize, blockSize>>>(d_A, d_B, d_C, ROWS, COLS);
